...
public class SessionTableController_GRIP : MonoBehaviour
{
    [Header("Table Wiring")]
    ...
    private readonly List<SessionRow_GRIP> rows = new();
    private SessionRow_GRIP selected;
    private SessionRow_GRIP lastClicked;
    ...
    void Awake()
    {
        if (!content)   Debug.LogError("[SessionTableController_GRIP] Content not assigned!", this);
        if (!rowPrefab) Debug.LogError("[SessionTableController_GRIP] Row Prefab not assigned!", this);
    }
    ...
    public void AddTrial()
    {
        ...
        var row = go.GetComponent<SessionRow_GRIP>();
        row.Init(this);
    ...
    public void SelectRow(SessionRow_GRIP row)
    {
        SelectRow(row, false);
    }

    public void SelectRow(SessionRow_GRIP row, bool shift)
    {
    ...
    public void SaveCsv()
    {
        ...
        string file = $"session_grip_{System.DateTime.Now:yyyyMMdd_HHmmss}.csv";
        ...
        Debug.Log($"[SessionTableGRIP] Saved CSV → {path}");
    }
    ...
    IEnumerator LoadCsvRoutine()
    {
        ...
        yield return FileBrowser.WaitForLoadDialog(
            FileBrowser.PickMode.Files, false, initial, "Load GRIP session CSV", "Load");

        if (!FileBrowser.Success) { SetStatus("Load canceled."); yield break; }

        string path = FileBrowser.Result[0];
        Debug.Log($"[SessionTableGRIP] Loading: {path}");
        ...
        try
        {
            ...
            if (skipped > 0)
                Debug.LogWarning($"[SessionTableGRIP] Skipped {skipped} rows with fewer than 10 columns.");

            nextIndex = rows.Count + 1;
            SelectRow(null);
            SnapshotToCache();
            SetStatus($"Loaded {rows.Count} trials.");
            Debug.Log($"[SessionTableGRIP] Loaded CSV ← {path}  ({rows.Count} rows, {skipped} skipped)");
        }
        catch (System.Exception e)
        {
            SetStatus($"Load error: {e.Message}");
            Debug.LogError($"[SessionTableGRIP] Load failed: {e}");
        }
    }

    // -------- Helpers --------

    string RowToCsv(int trialNum, SessionRow_GRIP r)
    {
    ...
    void SetStatus(string msg)
    {
        ...
        Debug.Log($"[SessionTableGRIP] {msg}");
    }
