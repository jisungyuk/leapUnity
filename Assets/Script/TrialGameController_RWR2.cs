...
/// </summary>
public class TrialGameController_RWR2 : MonoBehaviour
{
    ...
    [SerializeField] float  ttlPulseDurationMs = 100f;  // matches Python reference (TRIG_PULSE_MS=100)
    ...
    void OpenTtlPort()
    {
        ...
        try
        {
            ...
            Debug.Log($"[TrialGameController_RWR2] TTL port {ttlComPort} opened.");
        }
        catch (Exception e)
        {
            Debug.LogWarning($"[TrialGameController_RWR2] Could not open TTL port {ttlComPort}: {e.Message}");
            ttlPort = null;
        }
    }
    ...
    IEnumerator ResetTtlAfterDelay(float delaySeconds)
    {
        ...
        if (ttlPort != null && ttlPort.IsOpen)
        {
            ...
            catch (Exception e) { Debug.LogWarning($"[TrialGameController_RWR2] TTL reset failed: {e.Message}"); }
        }
    }
    ...
    void ShowDirectionCue()
    {
        ...
        ttlPending      = ttlEnabled;
        ttlFired        = false;
        ttlFiredTime    = -1f;

        // Arm LabChart FRO: Output delays are absolute from the TTL trigger.
        ...
        if (ttlEnabled && froController != null)
        {
            ...
            {
                Debug.LogWarning($"[TrialGameController_RWR2] Output1 absolute delay out of range: {out1Abs:F1}ms. FRO skipped.");
            }
            else if (doublePulse && (out2Abs < 0f || out2Abs >= 9900f))
            {
                Debug.LogWarning($"[TrialGameController_RWR2] Output2 absolute delay out of range: {out2Abs:F1}ms. FRO skipped.");
            }
            else
            {
                StartCoroutine(froController.PrepareOutputs(out1Abs, out2Abs, doublePulse));
            }
        }
    }
    ...
    void ResetToMoveToStart()
    {
        ...
        Debug.Log("[TrialGameController_RWR2] False start — reset to MoveToStart.");
    }
    ...
    void FireTtlPulse()
    {
        ...
        Debug.Log($"[TrialGameController_RWR2] TTL fired — {(ttlFiredTime - goTime) * 1000f:F1} ms from Go (target offset: {ttlOffsetMs} ms)");
        ...
        if (ttlPort != null && ttlPort.IsOpen)
        {
            ...
            {
                Debug.LogWarning($"[TrialGameController_RWR2] TTL write failed: {e.Message}");
            }
        }
    }
