...
public class TargetTableController_GRIP : MonoBehaviour
{
    [Header("Table Wiring")]
    [SerializeField] Transform  content;
    [SerializeField] GameObject rowPrefab;   // TargetRow_GRIP prefab

    [Header("Status (optional)")]
    [SerializeField] TMP_Text statusText;

    private readonly List<TargetRow_GRIP> rows = new();
    private TargetRow_GRIP selected;
    private int nextId = 1;

    void Awake()
    {
        if (!content)   Debug.LogError("[TargetTableController_GRIP] Content not assigned!", this);
        if (!rowPrefab) Debug.LogError("[TargetTableController_GRIP] Row Prefab not assigned!", this);
    }
    ...
    void Start()
    {
        ...
        if (store != null && store.RwrTargets.Count > 0)
        {
            ...
            SetStatus("Restored GRIP targets from memory");
        }
    }
    ...
    public void AddTarget()
    {
        ...
        var row = go.GetComponent<TargetRow_GRIP>();
        row.Init(this);
        row.SetId(nextId++);
    ...
    public void SelectRow(TargetRow_GRIP row)
    {
        selected = row;
    ...
    public void SaveCsv()
    {
        ...
        string file = $"targets_grip_{System.DateTime.Now:yyyyMMdd_HHmmss}.csv";
        ...
        Debug.Log($"[TargetTableGRIP] Saved → {path}");
    }
    ...
    IEnumerator LoadCsvRoutine()
    {
        ...
        yield return FileBrowser.WaitForLoadDialog(
            FileBrowser.PickMode.Files, false, initial, "Load GRIP targets CSV", "Load");
